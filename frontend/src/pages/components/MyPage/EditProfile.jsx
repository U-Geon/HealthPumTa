...
const EditProfile = () => {
  ...
  useEffect(() => {
    axios
      .get("/myPage") // /api/login은 백엔드의 로그인 엔드포인트 주소입니다.
      .then((response) => {
        // 서버 응답 처리
  ...
  const editComplete = () => {
    ...
    axios
      .post("/myPage/edit", editedUserInfo)
      .then((response) => {
        // 서버 응답 처리
        console.log(response.data);
        navigate("Main");
  ...
  return (
    <form onSubmit={handleSubmit}>
      <FormStyle>
        <div>
          <div
            style={{ marginTop: "5vh", marginBottom: "20px", fontSize: "23px" }}
          >
        ...
        <div style={{ display: "flex", justifyContent: "center" }}>
          <Completebtn
            style={{ cursor: "pointer", width: "295px", marginRight: "12px" }}
            type="submit"
        ...
        <ButtonWrapper>
          <ButtonRow>목표</ButtonRow>
          <ButtonRow>
            <CustomLink to="/Main">타이머</CustomLink>
          </ButtonRow>
          <ButtonRow color="#4E4FEB">내정보</ButtonRow>
        </ButtonWrapper>
      </FormStyle>
    </form>
  );
